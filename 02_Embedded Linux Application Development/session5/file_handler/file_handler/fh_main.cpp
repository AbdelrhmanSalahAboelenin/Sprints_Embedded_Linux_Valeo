...
int main()
{
    ...
    while (1)
    {
        ...
        // Check if data is available for reading from the named pipe
        ...
        if (ready > 0 && FD_ISSET(fd, &readSet))
        {
            ...
            // Check if reading from the pipe was successful
            if (count > 0)
            {
                ...
                else
                {
                    ...
                    if (fd < 0)
                    {
                        report_and_exit("Can't open the shared mem segment...");
                        LOG_ERROR << "Can't open the shared mem segment...";
                    }
                    ...
                    // Perform the appropriate command based on the value
                    if (value == 0)
                    {
                      // Handle read command
